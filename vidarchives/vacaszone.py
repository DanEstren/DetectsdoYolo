...
while cap.isOpened():
    ...
    for xyxy, track_id in zip(detections.xyxy, detections.tracker_id):
        ...
        if cv2.pointPolygonTest(counting_area, center_point, False) > 0:
            ...
            if track_id not in counted_cow_ids:
                ...
                cow_count += 1
                print(f"Vaca {track_id} contada! Total: {cow_count}")
    ...
    cv2.putText(exibir, f"Vacas contadas: {cow_count}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    
    # Desenha o polígono

    cv2.polylines(exibir, pts=[counting_area], isClosed=True, color=(0, 255, 0), thickness=3)
